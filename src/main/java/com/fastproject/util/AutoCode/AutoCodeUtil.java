package com.fastproject.util.AutoCode;

import cn.hutool.core.date.DateTime;
import com.fastproject.model.custom.autocode.AutoCodeConfig;
import com.fastproject.model.custom.autocode.AutoConfigModel;
...
public class AutoCodeUtil {
    ...
    for (String template : templates) {
      try {
        ...
        String filepath = getCoverFileName(template, tableInfo,
            autoCodeConfig.getConfigkey("parentPack"), targetPath);
  ...
  public static Map<String, String> viewAuto(TableInfo tableInfo, AutoConfigModel autoConfigModel) {
    ...
    for (String template : templates) {
      ...
      velocityMap.put(
          template.substring(template.lastIndexOf("/") + 1, template.lastIndexOf(".vm")),
          sw.toString());
    }
    return velocityMap;
  }
  ...
  public static void autoCodeOneModel(TableInfo tableInfo, AutoConfigModel autoConfigModel,
    ...
    for (String template : templates) {
      try {
        String filepath = getCoverFileName(template, tableInfo,
            autoCodeConfig.getConfigkey("parentPack"), "");
        Template tpl = Velocity.getTemplate(template, "UTF-8");
        StringWriter sw = new StringWriter();
        tpl.merge(velocityContext, sw);
  ...
  public static String getCoverFileName(String template, TableInfo tableInfo, String packageName,
      String targetPath) {
    ...
    String resourcesPath =
        targetPath + separator + "src" + separator + "main" + separator + "resources" + separator;
    ;
    ...
    if (template.contains("Entity.java.vm")) {//model.java
      return packagePath + "model" + separator + "auto" + separator + tableInfo.getJavaTableName()
          + ".java";
    }
    ...
    if (template.contains("EntityMapper.java.vm")) {//daomapper.java
      return packagePath + "mapper" + separator + "auto" + separator + tableInfo.getJavaTableName()
          + "Mapper.java";
    }
    if (template.contains("EntityMapper.xml.vm")) {//daomapper.xml
      return resourcesPath + "mybatis" + separator + "auto" + separator
          + tableInfo.getJavaTableName() + "Mapper.xml";
    }
    ...
    if (template.contains("EntityController.java.vm")) {
      return packagePath + "controller" + separator + "gen" + separator
          + tableInfo.getJavaTableName() + "Controller.java";
    }
    if (template.contains("list.html.vm")) {
      return resourcesPath + "templates" + separator + "gen" + separator
          + tableInfo.getJavaTableName_a() + separator + "list.html";
    }
    if (template.contains("add.html.vm")) {
      System.err.println(resourcesPath + "templates" + separator + "gen" + separator
          + tableInfo.getJavaTableName_a() + separator);
      return resourcesPath + "templates" + separator + "gen" + separator
          + tableInfo.getJavaTableName_a() + separator + "add.html";
    }
    if (template.contains("edit.html.vm")) {
      return resourcesPath + "templates" + separator + "gen" + separator
          + tableInfo.getJavaTableName_a() + separator + "edit.html";
    }
